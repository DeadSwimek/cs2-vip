...
public partial class VIP : BasePlugin, IPluginConfig<ConfigVIP>
{
    ...
    public void LoadPlayerData(CCSPlayerController player)
    {
        ...
        if (result.Rows == 1)
        {
            ...
            Server.PrintToConsole($"Player {player.PlayerName} ({player.SteamID}) have VIP. Remaining time of VIP {timeRemainingFormatted}");
            // Checking if is still time to VIP
            if (result.Get<int>(0, "end") != 0)
            {
                if (timeRemaining.Days < 0)
                {
                    Server.PrintToConsole($"Player {player.PlayerName} ({player.SteamID}) exp. VIP today..");
            ...
            {
                Server.PrintToConsole($"Player {player.PlayerName} ({player.SteamID}) have VIP forever");
            }
        }
        else
        {
            Server.PrintToConsole($"Player {player.PlayerName} ({player.SteamID}) is not VIP");
        }
    }
